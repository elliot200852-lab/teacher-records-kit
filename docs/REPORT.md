...
### `students/{代號}` —— 每位學生一份摘要
| 欄位 | 型別 | 說明 |
|---|---|---|
| `id` | string | 學生代號（如 `S-01`） |
| `recordCount` | number | 這位學生累積幾則紀錄 |
| `lastRecordDate` | string | 最後一次記錄的日期 |
| `monthsRecorded` | array | 有記錄過的月份（`YYYY-MM`），用來算「本月已記／未記」 |

> 摘要欄位由進階同步腳本維護；只用網頁的話，儀表板會直接數 `records` 子集合。
