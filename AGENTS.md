...
> 若他還有**第二個帳號要一起登入**——例如協同帶班的另一位老師、或他自己的備用信箱，
> 想要跟這個信箱權限完全一樣（一樣登得進網站、一樣看得到全部資料，**不是分權**，
> 只是同一個擁有者多一個入口）：這**不是安裝精靈會主動問的題目**，先把信箱記下來，
> 等步驟 4 寫進答案檔 `answers.json` 的 `co_owner_emails`（信箱清單），
> 或事後直接改 `config/kit.json` 補上，重跑一次 `build_config.py` 並重新部署規則。
...
- **第一欄**可以寫座號（`01`，程式會自動接上前綴變成 `S-01`），也可以直接寫完整代號（`S-01`）。
  **前綴本身也可以帶數字**，例如班級代號 `4A`——代號就會是 `4A-01`、`4A-02`……
  （`alpha.5` 修過一個「前綴含數字會被讀壞」的 bug：以前 `4A-01` 會被誤判成裸數字變成
  `4A-401`，現在 `4A-01`／`4A-2`／`03` 三種寫法都能正確讀回 `4A-0N`。）
...
> **只記「導師班級學生紀錄」（`homeroom`，沒有「報告維度」欄位）的老師，一樣做得出期末評量。**
> 五個評量維度（行為與自我管理／人際互動／學習態度與能力／內在特質與個人發展／挑戰與方向）
> 這時候是系統從**標籤（`tagMap`）＋內文關鍵詞（`keywords`）**自動推導的
> （表在 `config/report-formats.library.json` 的 `waldorf-homeroom`，網頁零硬編；一則可以同時落好幾個維度）。
> 老師記錄的當下就看得到缺口：學生卡上五個小籤（0 則紅／1–2 則黃／3+ 則綠）、
> 明細頁「評量維度覆蓋」卡（點維度可以過濾、「補一則」會預填代表標籤）。
> **素材包按推導出的維度分組**，某個維度這一年真的一則都沒有，素材包會明寫
> 「這一年沒有觀察到，期末這段寫不出來」，不省略、不含糊帶過——
> 也要提醒老師：維度是機器推的，寫評語時仍要回頭看正文，不是看到綠燈就代表寫得出好內容。
...
3. **重新部署安全規則**（v3 加了業務記錄與狀態列的區塊——**這一步不能省**）：
   ```bash
   firebase deploy --only firestore:rules --project <他的專案ID>
   ```
   **升到 v3.0.0-alpha.5 或更新的版本，這一步格外不能省**：這一版把刪除改成兩段式
   （網頁上按刪除是軟刪，規則的 `delete` 一律拒絕，真刪只有老師自己在終端機跑
...
- 刪除按不動並說規則是舊版 → 同上，重新部署規則。**alpha.5 起「刪除」在網頁上一律是軟刪**
  （雲端原文還在，只從畫面／本機／備份消失），規則沒更新的話這個操作會直接被拒絕。
...
2. **同一位學生在不同記錄類型各有一份紀錄，互不相干。**
   ...
   **課程是例外**：在網頁上新建的課程，下一次 `sync.py` 會自動幫它在本機建好
   `data/courses/<課程id>/records.md`，不用你手動介入，`sync.py` 只會印一行提醒多了哪一門課。
...
6. **一鍵匯出 Word／PDF。** 期末、家長會、個案會議要交一份能印出來的東西時，不用重打。
   ...
   **在網頁上按就好（手機也行）**——四個落點：
   - **學生明細頁**右上角的「**匯出 ▾**」：Word 與 PDF 各兩項——
     「只匯出目前這一種記錄類型」或「這位學生全部類型」；同一個選單另外多一項
     **「Markdown（.md，給 AI）・全部類型」**。
   - **課程明細頁、業務組明細頁**是同一顆「**匯出 ▾**」，三項：
     「匯出 Word（.doc）」「匯出 PDF（列印）」「**Markdown（.md，給 AI）**」（班級整體觀察那一頁也有同一顆）。
   - **學生分頁一覽**最上面的「**匯出所有學生 ▾**」：勾要哪幾種記錄類型、
     填日期範圍（兩格都留空＝全部日期），可以再勾「含班級整體觀察（放在文件最前面）」；
     出來是一份文件、每位學生一節（姓名＋代號），沒有記錄的學生也會列一行「（無記錄）」；
     面板裡「匯出 Word」「匯出 PDF」旁邊還有一顆「**匯出 .md（給 AI）**」。
   - **課程一覽頁**最上面多一顆「**全部課程匯出 .md**」：一份檔、每門課一節
     （整體課程紀錄放最前面、逐日紀錄接在後面），期末寫課程總結、交課程計畫成果時直接整份貼給 AI。

   **Markdown（.md）是給 AI 讀的匯出格式**：格式跟本機 `data/*.md` 一樣（代號、標籤、欄位、正文），
   正文一律代號，**真名只列在檔尾「附錄：代號對照」**，檔案最前面標一行
   「唯讀副本，不要複製回 data/」——它是匯出當下的快照，不是可以編輯回寫的檔。
...
7. **刪除了的東西去哪裡了。** 網頁上按「刪除」是**兩段式**的：老師按的那一下是**軟刪**——
   那一則從畫面、本機 `data/*.md`、下一次備份裡消失，但雲端 Firestore 上的原文**還留著**，
   規則已經把真正的 `delete` 一律拒絕（前端沒有辦法真刪）。同步、匯出、期末素材包、
   台帳計數一律當它不存在；只有 `backup.py` 的雲端全量快照 `export.json` 故意保留它，
   萬一是誤刪還找得回來。
   **真的要把雲端原文永久抹掉，只有一條路**：**老師本人**在終端機跑
   `python3 scripts/purge_deleted.py`——不帶參數（或 `--list`）只列出已刪還留著的東西
   （對象／rid／刪除時間／字數，**不印正文**）；要刪一則帶
   `--rid <id> --target <種類/代號> --confirm`，全部清掉帶 `--all --confirm`；
   **沒有 `--confirm` 一則都不會動**。它走的是老師自己的 `gcloud` 使用者權杖，
...
| `scripts/purge_deleted.py` | **只有老師本人在終端機跑**：把軟刪的紀錄從雲端永久抹掉，不可逆（`--list`／`--rid …--target …--confirm`／`--all --confirm`） |
