...
- 網頁每個明細頁的「匯出 ▾」多一項 **Markdown（.md）**：學生＝該生全部記錄類型、課程、班級觀察、
  業務組；全班面板多「匯出 .md（給 AI）」（全班所有學生所有類型一個檔）；課程一覽多「全部課程匯出 .md」。
  格式與本機 `data/*.md` 同形（`## 日期 時間 #標籤`、`欄位：值`、正文），但檔首有「唯讀副本，不要複製回 data/」
  標記與隱私提醒；正文一律代號，真名只在檔尾「附錄：代號對照」。

### 整體課程紀錄

- 每門課多一段**不分天**的整體紀錄：課程明細頁一張「整體課程紀錄」卡（檢視／編輯／儲存，上限 20000 字），
  課程一覽卡有寫的顯示「整體紀錄 ✓」。雲端欄位 `courses/{id}.overview`（＋`overviewUpdatedAt`），
  本機 `data/courses/<id>/card.json`，`sync.py` 雙向同步（本機推上去只動 overview；衝突時兩邊都不覆蓋、記 note）。
  Word／PDF／.md 匯出都在逐日紀錄前先放這一段。**規則不用改**（課程卡本來就是擁有者全權讀寫）。
- 順手補一個缺口：**在網頁新增的課程**以前本機完全看不到（不進 sync、備份、匯出）；現在 `sync.py`
  會自動建 `data/courses/<id>/records.md` 並記 note。
- 測試 270→284；三個測試類補 tempdir 清理（以前每跑一次留 20 個 `trk-test-*`）。

### 單一紀錄流＋五維度自動推導（照 5A 導師儀表板）

- 只記一種自由文字的學生紀錄（homeroom，只有 #標籤沒有欄位）也能做期末評量：五個評量維度
  從 **#標籤（`tagMap`）＋內文關鍵詞（`keywords`）** 自動推導，一則可落多個維度。三張表住在
  `config/report-formats.library.json` 的 `waldorf-homeroom`（`tagMap`／`keywords`／`thinMax`），
  網頁零硬編；只對**沒有「報告維度」欄位**的類型啟用，有欄位的類型行為不變。
- 記錄當下就看得到缺口：學生卡五個小籤（0 紅／1–2 黃／3+ 綠）、工具列「只看評量維度還沒齊的」、
  全班摘要「○○維度：N 位還缺」、明細頁「評量維度覆蓋」卡（點維度過濾、「補一則」預填代表標籤）、
  新增表單第二排維度快速鍵（缺的那顆虛線框）、每張紀錄卡底下寫「算進：…（命中：…）」。
- 期末素材包按推導維度分組，每段先寫「這一年這維度有 N 則」；**0 則的維度明寫
  「這一年沒有觀察到，期末這段寫不出來」**，不省略；開頭 prompt 說明維度是機器推的、以正文為準。
- `site/js/kit-config.example.js` 同步三張表（離線預覽用），加測試擋兩份漂移。測試 284→287。

### 修正

- **名冊代號的前綴含數字會被讀壞**：`data/roster.csv` 寫 `4A-01`，`lib.load_roster_rows` 用 `\D+` 抓前綴
  抓不到，退回「裸數字」路徑變成 `4A-401`——doctor 報缺 48 個檔、sync 會把整班推成錯的代號。
  文件一直說「想用班級當前綴也可以，例如 6B」，所以這是真的會咬到人的 bug。改成 `(.+?)-0*(\d+)`，
  `4A-01`／`4A-2`／`03` 三種寫法都回到 `4A-0N`。（2026-09-13 第一次真雲端安裝踩到）
